...
exports.get = function(dbinfo, secName, ptKey, id, callback) {
    var model = dbinfo.matchModels[secName];
    var query = model.findOne({_id: id, pat_key: ptKey}).populate('entry match_entry').lean();
    query.exec(function (err, result) {
        if (err || !result) {
            logger.error(err|| 'no match found');
            err ? callback(err) : callback(new Error('no match found'));
        } else {

...
exports.get = function (dbinfo, secName, ptKey, id, callback) {
    ...
    if (ptKey) { // todo: security problem?
        queryInput.pat_key = ptKey;
    }
...
var updateOrReplace = (function () {
    ...
    return function (dbinfo, secName, ptKey, id, sourceId, updateObject, replace, callback) {
        var model = dbinfo.models[secName];
        var queryObject = {
            '_id': id,
            pat_key: ptKey
...
exports.update = function (dbinfo, secName, ptKey, id, sourceId, updateObject, callback) {
    updateOrReplace(dbinfo, secName, ptKey, id, sourceId, updateObject, false, callback);
};

exports.replace = function (dbinfo, secName, ptKey, id, sourceId, updateObject, callback) {
    updateOrReplace(dbinfo, secName, ptKey, id, sourceId, updateObject, true, callback);
};
...
exports.idToPatientKey = function (dbinfo, secName, id, callback) {
    ...
    query.exec(function (err, current) {
        if (err | !current) {
            if (!err) {
                err = new Error('no entry found.');
            }
            callback(err);
        } else {
            callback(null, current.pat_key);
        }
    });
};

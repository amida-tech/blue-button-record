...
var dbObjectToExternal = function (dbObject) {
  ...
  r.metadata = dbObject.metadata;
  r.merged = dbObject.merged;
  if (dbObject._link) {
    r._link = dbObject._link.toString();
  }
  if (dbObject._resource) {
    r._resource = dbObject._resource;
  }
  return r;
};
...
var searchAllIdsForSection = function (dbinfo, sectionName, searchSpec, callback) {
  ...
  if (searchSpec.resource) {
    query.where('_resource', searchSpec.resource);
  } else {
    query.where('_resource', null);
  }
  query.sort({
    _id: -1
  });
  ...
  var fn = function (entry, cb) {
    ...
    model.findOne({
      ...
      if (err) {
        ...
        var attr = result.metadata.attribution;
        var mergeId = attr && attr[attr.length - 1];

        var mergeModel = dbinfo.mergeModels[sectionName];
        var query = mergeModel.find({
          _id: mergeId
        });
        query.lean();
        query.select('merged');
        query.exec(function (err, mergeDatas) {
          if (err) {
            cb(err);
          } else {
            // attr[attr.length - 1] = {
            //   merged: mergeDatas[0].merged
            // };
            cb(null, result);
          }
        });
